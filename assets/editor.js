...
(function () {
      ...
      if (!already) {
        Blocks.registerBlockType("nowonline/elt-template", {
          ...
          edit: function (props) {
            ...
            // ContentTab: KUN picker + stort preview + felter
            function ContentTab() {
              var tpl   = tplById(templateId);
              var thumb = (tpl && tpl.thumb) ? tpl.thumb : "";

              var pickerRow = el("div", { className: "now-elt-picker" }, picker);

              var previewBox = tpl ? el("div", { className: "now-elt-canvas-preview" },
                el("div", {
                  className: "now-elt-preview-img-wrap" + (thumb ? "" : " now-elt-preview-placeholder")
                },
                  thumb
  ? el("img", {
      src: thumb,
      ...
      // failguard: force full image – no cropping
  ...
  : el("span", {}, " ")
            ...
            function BackgroundTab() {
              function onSelect(media) { setBackground({ image: (media && media.url) || "" }); }
              function clear()        { setBackground({ image: "" }); }
              var preview = background.image ? el("img", { src: background.image, className: "now-elt-imgprev", alt: "" }) : null;

              return el("div", { className: "now-elt-wrap" },
                el("div", { className: "now-elt-sec-title" }, __("Baggrundsbillede", "nowonline")),
                preview,
                ...
                el(SelectControl, {
                  ...
                  onChange: function (v) { setBackground({ position: v }); },
                  options: [
                ...
                el(SelectControl, {
                  ...
                  onChange: function (v) { setBackground({ size: v }); },
                  options: [
                ...
                el(ToggleControl, {
                  ...
                  onChange: function (v) { setBackground({ fixed: !!v }); }
                })
              );
            }
            ...
            function AdvancedTab() {
              return el("div", { className: "now-elt-wrap" },
                el("div", { className: "now-elt-sec-title" }, __("Responsiv synlighed", "nowonline")),
                el(CheckboxControl, {
                  label: __("Skjul på desktop", "nowonline"),
                  checked: !!responsive.hideDesktop,
                  onChange: function (v) { setResponsive({ hideDesktop: !!v }); }
                }),
                el(CheckboxControl, {
                  label: __("Skjul på tablet", "nowonline"),
                  checked: !!responsive.hideTablet,
                  onChange: function (v) { setResponsive({ hideTablet: !!v }); }
                }),
                el(CheckboxControl, {
                  label: __("Skjul på mobil", "nowonline"),
                  checked: !!responsive.hideMobile,
                  onChange: function (v) { setResponsive({ hideMobile: !!v }); }
                }),
                el("hr", {}),
                el(RangeOrNumber, {
                  ...
                  onChange: function (v) { setSpacing({ pt: parseInt(v || 0, 10) || 0 }); }
                }),
                el(RangeOrNumber, {
                  label: __("Padding bottom (px)", "nowonline"),
                  min: 0, max: 200, step: 1,
                  value: parseInt(spacing.pb || 0, 10),
                  onChange: function (v) { setSpacing({ pb: parseInt(v || 0, 10) || 0 }); }
                })
              );
            }
