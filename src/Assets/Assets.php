...
private function enqueue_editor(string $ver): void
{
    ...
    // 🔹 Inline boot-data (SKAL ligge før selve filen kører)
    // 1) LivePreview bridge (base-URL + nonce)
    $site  = home_url('/');
    $nonce = wp_create_nonce(\NowOnline\EltBlocks\Rendering\LivePreview::QV);

    // 2) Templates + feltdefinitioner
    $boot  = ''
        . 'window.NOWONLINE_ELT=' . wp_json_encode(['site'=>$site,'nonce'=>$nonce]) . ';'
        . 'window.NOWONLINE_TEMPLATES=' . $map_json . ';'
        . 'window.NOWONLINE_FIELDS='    . $fmap_json . ';';

    wp_add_inline_script('nowonline-elt-blocks-js', $boot, 'before');

    // Enqueue JS
    wp_enqueue_script('nowonline-elt-blocks-js');

    // Editor CSS (preview + større inserter-tiles)
